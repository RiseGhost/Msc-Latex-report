...
como tal existe um afastamento da zona de \textit{flow}, uma vez que devida a adaptação do jogador, o desafio se torna mais fácil.
...
estas cinco etapas, o design procura favorecer a manutenção do jogador numa zona de \textit{flow}, 
...
não estão contemplados no projeto desenvolvido, uma vez que não eram o foco principal e
...
Quando é necessário transmitir uma quantidade elevada de informação ao jogador, as notificações não suprem essa demanda uma vez que são pequenas,
o que não permite mostrar quantidades elevadas de informação, além de não serem persistentes.

Com o intuito de possibilitar com que o jogador consiga receber quantidades maiores de informação de forma persistente é que foi criado um sistema de email. Uma vez que desta
forma o jogador dispõe uma área maior para exibir informações além de poder, consultar as informações a qualquer momento da \textit{gameplay}.

Além disso, é uma forma diegética do sistema do jogo interagir com o jogador. Sempre que o jogador recebe um email, é-lhe notificado com uma notificação, e é ativado
um indicador a informar que há emails não lidos, como pode ser vista na figura \ref{Email_1}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/Email Noti.png}
    \caption{Email - notificação de novo email.}
    \label{Email_1}
\end{figure}

Para o jogador conseguir aceder ao seu email, ele têm que se dirigir ao seu portátil, de seguida interagir com mesmo e depois navegar na \ac{UI}, que foi inspirada na \ac{UI} de um computador.
Este processo foi idealizado desta forma, para se manter fiel ao processo. A \ac{UI} do email dentro do portátil pode ser vista na figura \ref{Email_2}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/Email UI.png}
    \caption{Email - \ac{UI}.}
    \label{Email_2}
\end{figure}
...
O menu de pausa é algo bastante comum em vídeo jogos, geralmente aparece quando o jogo fica temporariamente parado
e normalmente disponibiliza algumas opções ao jogador.

Geralmente é utilizado quando o jogador pretende sair do jogo ou voltar ao algum menu. No projeto desenvolvido o menu de pausa foi
utilizado para mostrar algumas informações adicionais, tais como o conjunto de teclas utilizadas pelo jogo associadas à sua funcionalidade
e o dinheiro atual do jogador. Como pode ser visto na Figura \ref{PauseMenu}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/Menu de Pausa.jpg}
    \caption{Menu de Pausa - \ac{UI}.}
    \label{PauseMenu}
\end{figure}

O motivo de ter utilizado este espaço da \ac{UI} para colocar tal informação deve-se a sua premissa, uma vez que caso o jogador não se lembra de alguma
tecla, quando for colocar o jogo de pausa, quer seja para sair ou para outro fim, terá essa informação.

\subsection{Referências para a criação de elementos de Jogo e 3D}
\label{Sec:ReferenciasReais}
...
A representação no jogo é uma adaptação do pavimento técnico real, onde as caixas foram simplificadas, e a fiação de cabos foi desconsiderada. A sua localização no cenário, também serve de marcação para o jogador, uma vez que sempre que existe a compra de um novo servidor, o mesmo é colocado atrás de uma dessas caixas.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/UI_Gnome.jpg}
    \caption{UI da instalação e configuração de um sistema operativo VS ambiente gráfico Gnome.}
    \label{UI_Gnome}
\end{figure}

Por fim, além de elementos gráficos de cenário, elementos de \textit{UI}, tais como a interface de configuração de um sistema operativo, foram igualmente inspirados em referências reais.
Na Figura \ref{UI_Gnome} (do lado direito) é mostrado a interface gráfica Gnome, que é um ambiente gráfico vastamente utilizado em sistemas operativos Linux, embora a sua utilização em data centers não seja tão comum, ainda assim, é bastante dissolvida no ambiente de tecnologia.
Por apresentar uma interface gráfica mais acessível e intuitiva, o ambiente GNOME serviu de referência para a construção da interface vista na Figura \ref{UI_Gnome} (do lado esquerdo).
...
por fazer a gestão de um pequeno data center, onde o objetivo é expandir o data center, onde o jogador terá de passar por várias etapas. Quando o jogador tiver o seu data center no máximo irá receber uma recompensa que será uma chave criptografica que controla o tempo. Este recompensa está intrinsecamente relacionada com a narrativa do jogo.  

Todavia, embora este seja o conceito idealizado, o objetivo final (a chave) não se encontra presente na versão final, uma vez que não existiu tempo para desenvolver um sistema de progressão claro que direciona-se o jogador para ele.

Como tal, durante todo o percurso de \textit{gameplay}, o jogador terá várias etapas a cumprir,
...
e o uso server para reforçar a imersão do jogador, além de tornar o jogo mais
...
Sendo assim as únicas despesas presentes tais como:
\begin{itemize}
    \item Custos relacionados com o consumo de eletricidade;
    \item Custos relacionados com a compra de equipamentos.
\end{itemize}

Todavia, o sistema de economia presente cumpre a função de transmitir
...
a sua disposição um sistema de combate e \textit{builds} semelhantes a jogos de \ac{RPG},
...
adaptado, no sentido em que em vez de ter o sistema de \ac{RPG}, o jogo terá uma
...
Além disso, permite integrar a idea de que os data centers são atacados e que é preciso tomar medidas de segunraça,
ainda que aqui se tenha simplificado este processo e simplesmente se possa eleminar os inimigos (i.e., os ataques que o data center enfreta)
...
Posto isto, nas subsecçõe seguintes serão apresentados o core \textit{gameplay loop}, assim como os \textit{gameplays} secundário.
...
A seguir apresenta-se os \textit{gameplay loops} secundários, ou seja, os ciclos de ações relacionados com a compra e configuração de equipamentos, e o ciclo de ações relacionado com o combate, como mostram as Figuras \ref{Servers_Gameplay_Loop} e \ref{Combat_Gameplay_Loop}, respetivamente.
